...
export default function TabelasPage() {
  ...
  async function loadAprovacoes(uid: string) {
    ...
    setAprovacoes(list)
    // track IDs of rows with pending approval (solicitante view)
  ...
  const thStyle: React.CSSProperties = { padding: '8px 12px', fontSize: 11, fontWeight: 600, color: '#888', textTransform: 'uppercase' as const, letterSpacing: '1px', background: '#f7f7f5', borderBottom: '0.5px solid #e5e5e5', textAlign: 'left' as const }
    ...
    <div style={{ minHeight: '100vh', background: '#f7f7f5' }}>
          ...
          <div style={{ background: '#fff', border: '0.5px solid #e5e5e5', borderRadius: 8, overflow: 'hidden' }}>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['solo_type', 'bioma', 'regiao', 'clay_pct', 'sand_pct', 'texture_class', ''].map(h => (
            ...
            {/* Adicionar linha */}
            {addingRow ? (
              <div style={{ padding: 16, borderTop: '0.5px solid #e5e5e5', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <div>
                ...
                <div>
                  ...
                  <input type="number" value={newRow.clay_pct as number || ''} onChange={e => setNewRow(v => ({ ...v, clay_pct: +e.target.value }))} style={{ ...inputSm, width: 70 }} />
                </div>
                <div>
                  <label style={{ fontSize: 11, color: '#888', display: 'block', marginBottom: 4 }}>sand_pct</label>
                  <input type="number" value={newRow.sand_pct as number || ''} onChange={e => setNewRow(v => ({ ...v, sand_pct: +e.target.value }))} style={{ ...inputSm, width: 70 }} />
                </div>
                <div>
          ...
          <div style={{ background: '#fff', border: '0.5px solid #e5e5e5', borderRadius: 8, overflow: 'hidden' }}>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['regiao', 'mês', 'threshold_descansado', 'threshold_saturado', ''].map(h => (
                      <th key={h} style={thStyle}>{h}</th>
                    ))}
                  </tr>
                </thead>
            ...
            {addingRow ? (
              <div style={{ padding: 16, borderTop: '0.5px solid #e5e5e5', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                ...
                <div>
                  ...
                  <input type="number" step="0.1" value={newRow.threshold_descansado as number || ''} onChange={e => setNewRow(v => ({ ...v, threshold_descansado: +e.target.value }))} style={{ ...inputSm, width: 90 }} />
                </div>
                <div>
                  <label style={{ fontSize: 11, color: '#888', display: 'block', marginBottom: 4 }}>threshold_saturado</label>
                  <input type="number" step="0.1" value={newRow.threshold_saturado as number || ''} onChange={e => setNewRow(v => ({ ...v, threshold_saturado: +e.target.value }))} style={{ ...inputSm, width: 90 }} />
                </div>
          ...
          <div style={{ background: '#fff', border: '0.5px solid #e5e5e5', borderRadius: 8, overflow: 'hidden' }}>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['solo_type', 'exposicao', 'meia_vida_h', ''].map(h => (
                      <th key={h} style={thStyle}>{h}</th>
                    ))}
                  </tr>
                </thead>
            ...
            {addingRow ? (
              <div style={{ padding: 16, borderTop: '0.5px solid #e5e5e5', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                ...
                <div>
                  ...
                  <input type="number" step="0.5" value={newRow.meia_vida_h as number || ''} onChange={e => setNewRow(v => ({ ...v, meia_vida_h: +e.target.value }))} style={{ ...inputSm, width: 80 }} />
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
      ...
      {modal && (
        <div style={{
          ...
          <div style={{ background: '#fff', borderRadius: 8, padding: 24, maxWidth: 480, width: '100%', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
